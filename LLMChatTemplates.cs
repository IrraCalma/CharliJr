...
namespace LLMUnity
{
    ...
    public class Qwen3Template : ChatMLTemplate
    {
        ...
        // 🔹 Llamas a esto desde tu UI:
        // Qwen3Template.SetGlobalThinkingMode(thinkingToggle.isOn);
        //
        // Regla que pediste:
        //  - toggle == true  -> thinkingMode = false
        //  - toggle == false -> thinkingMode = true
        public static void SetGlobalThinkingMode(bool toggleValue)
        {
            if (ChatTemplate.templates == null)
                return;

            if (ChatTemplate.templates.TryGetValue("qwen3", out ChatTemplate template))
            {
                if (template is Qwen3Template qwenTemplate)
                {
                    // IF / ELSE explícito, invirtiendo el valor del toggle
                    if (toggleValue)
                    {
                        // Toggle ON  -> thinkingMode false
                        qwenTemplate.thinkingMode = false;
                    }
                    else
                    {
                        // Toggle OFF -> thinkingMode true
                        qwenTemplate.thinkingMode = true;
                    }
                }
            }
        }
        ...
        // Valor interno que usa HasThinkingMode
        public bool thinkingMode = false;

        // Por si quieres setearlo directo en algún momento
        public void SetThinkingMode(bool value)
        {
            thinkingMode = value;
        }

        protected override bool HasThinkingMode()
        {
            return thinkingMode;
