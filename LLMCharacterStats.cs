/// @file
/// @brief File implementing the LLM characters with statistics.
using System;
...
namespace LLMUnity
{
    public static class TaskExtensions
    {
        ...
        {
            // This extension allows tasks to run without being awaited
            task.ContinueWith(t => 
            {
                if (t.IsFaulted)
                {
                    LLMUnitySetup.LogError($"Task failed: {t.Exception}");
                }
    ...
    /// @ingroup llm
    /// <summary>
    /// Class implementing the LLM characters with statistics tracking.
    /// </summary>
    public class LLMCharacterStats : LLMCaller
    {
        ...
        public int nKeep = -1;
        /// <summary> number of total tokens in the context window (provided by the LLM) </summary>
        [Tooltip("number of total tokens in the context window (provided by the LLM)")]
        public int contextSize = 2048; // Default value, will be updated from LLM
        ...
        /// <summary> the name for thinking messages (these won't be saved to context) </summary>
        [Tooltip("the name for thinking messages (these won't be saved to context)")]
        [Chat] public string thinkingName = "thinking";
        ...
        // Global static variables for accessing stats from anywhere
        public static float CurrentTokensPerSecond = 0f;
        public static int RemainingContextTokens = 0;
        public static int TotalUsedTokens = 0;

        // Variables for tracking streaming performance
        ...
        public override void Awake()
        {
            ...
            if (!remote)
            {
                ...
                // Get context size from LLM if possible
                if (llm != null)
                {
                    contextSize = llm.contextSize; // Access the public contextSize field directly
                }
            }
            InitGrammar();
            InitHistory();
        ...
        public virtual void ClearChat()
        {
            ...
            // Update context stats after clearing - call async method properly
            _ = UpdateContextStats();
        }
        ...
        {
            // Only add to chat history if it's not a thinking message
            if (role.ToLower() != thinkingName.ToLower())
            {
                // add the question / answer to the chat list, update prompt
                chat.Add(new ChatMessage { role = role, content = content });
                // Update context stats after adding message - call async method properly
                _ = UpdateContextStats();
            }
        }
        ...
        /// <summary>
        /// Allows to add a thinking message that won't be saved to context.
        /// </summary>
        /// <param name="content">message content</param>
        public virtual void AddThinkingMessage(string content)
        {
            // Add to chat list but don't include in context calculations
            chat.Add(new ChatMessage { role = thinkingName, content = content });
        }
        ...
        // Method to start token tracking during streaming
        private void StartTokenTracking()
        {
            tokensReceived = 0;
            streamStartTime = DateTime.Now;
            isStreaming = true;
            CurrentTokensPerSecond = 0f;
        }

        // Method to update token count during streaming
        public void UpdateTokenCount(int newTokens = 1)
        {
            if (isStreaming)
            {
                ...
                {
                    CurrentTokensPerSecond = tokensReceived / (float)elapsed.TotalSeconds;
                }
            }
        }

        // Method to stop token tracking
        private void StopTokenTracking()
        {
            ...
            {
                CurrentTokensPerSecond = tokensReceived; // Handle case where elapsed time is zero
            }
        }

        /// <summary>
        /// Method to calculate and update remaining context tokens
        /// </summary>
        public async void UpdateContextStats()
        {
            // Calculate approximate token usage from the current chat history
            int totalTokens = 0;
            
            // Count tokens in each message using the LLM's tokenizer if available
            foreach (var message in chat)
            {
                if (message.content != null && message.role != "system") // Don't count system prompt in this estimation if it's already handled separately
                {
                    try 
                    {
                        // Try to get actual token count using the LLM's tokenizer
                        List<int> tokens = await Tokenize(message.content);
                        if (tokens != null)
                        {
                        ...
                        {
                            // Fallback: simple estimation using character count
                            totalTokens += Mathf.CeilToInt(message.content.Length / 4.0f);
                        }
                    }
                    catch
                    {
                        // If tokenization fails, use character-based estimation
                        totalTokens += Mathf.CeilToInt(message.content.Length / 4.0f);
                    }
                }
            }
            ...
            RemainingContextTokens = Mathf.Max(0, contextSize - TotalUsedTokens);
        }

        /// <summary>
        /// Method to get current tokens per second during streaming
        /// </summary>
        /// <returns>Current tokens per second as a float</returns>
        ...
        /// <summary>
        /// Method to get remaining context tokens
        /// </summary>
        /// <returns>Number of remaining tokens in the context window</returns>
        public int GetRemainingContextTokens()
        {
            return RemainingContextTokens;
        }

        /// <summary>
        /// Method to get total used tokens
        /// </summary>
        /// <returns>Total number of tokens used in the context</returns>
        ...
        public virtual async Task<string> Save(string filename)
        {
            ...
            // Only save non-thinking messages to the history
            List<ChatMessage> nonThinkingMessages = new List<ChatMessage>();
            foreach (var msg in chat)
            {
                if (msg.role.ToLower() != thinkingName.ToLower())
                {
                    nonThinkingMessages.Add(msg);
                }
            }
            string json = JsonUtility.ToJson(new ChatListWrapper { chat = nonThinkingMessages.GetRange(1, nonThinkingMessages.Count - 1) });
            ...
            else
            {
                ...
                if (stream && callback != null)
                {
                    if (typeof(Ret) == typeof(string))
                    {
                        ...
                        {
                            // Update token count for each callback during streaming
                            if (!string.IsNullOrEmpty(strArg))
                            {
                                // Try to get actual token count using the LLM's tokenizer for better accuracy
                                Task.Run(async () =>
                                {
                                    try
                                    {
                                        List<int> tokens = await Tokenize(strArg);
                                        if (tokens != null)
                                        {
                                        ...
                                        {
                                            // Fallback: estimate tokens based on word count (simple approximation)
                                            int approxTokens = Mathf.Max(1, strArg.Split(' ', '\n', '\t').Length);
                                            UpdateTokenCount(approxTokens);
                                        }
                                    }
                                    catch
                                    {
                                        // If tokenization fails, use word-based estimation
                                        int approxTokens = Mathf.Max(1, strArg.Split(' ', '\n', '\t').Length);
                                        UpdateTokenCount(approxTokens);
                                    }
                                }).FireAndForget(); // Fire and forget the token counting
                            }
